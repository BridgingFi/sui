...
# Script to cancel a deposit request
# Usage: ./cancel_deposit.sh <VAULT_ID> <REQUEST_ID> <OPERATOR_CAP_ID> <RECEIPT_ID> <RECIPIENT>
#   - VAULT_ID: The vault object ID
#   - REQUEST_ID: The deposit request ID (u64)
#   - OPERATOR_CAP_ID: The OperatorCap object ID
#   - RECEIPT_ID: Receipt ID (address). Required.
...
if [ -z "$VAULT_ID" ]; then
  ...
  echo "Usage: $0 <VAULT_ID> <REQUEST_ID> <OPERATOR_CAP_ID> [RECEIPT_ID] [RECIPIENT]"
  exit 1
fi
...
echo "Canceling Deposit Request"
echo "Vault ID: $VAULT_ID"
echo "Request ID: $REQUEST_ID"
echo "OperatorCap ID: $OPERATOR_CAP_ID"
echo ""

# Step 1: Query vault object to get coin type
...
# Step 2: Cancel deposit
echo "Step 2: Canceling deposit transaction..."
echo "Calling: ${VOLO_VAULT_PACKAGE_ID}::operation::cancel_user_deposit"
echo ""

if ! execute_sui_command "sui client call --package \"$VOLO_VAULT_PACKAGE_ID\" --module operation --function cancel_user_deposit --type-args \"$COIN_TYPE\" --args \"$VOLO_OPERATION_ID\" \"$OPERATOR_CAP_ID\" \"$VAULT_ID\" \"$REQUEST_ID\" \"$RECEIPT_ID\" \"$RECIPIENT\" \"$SUI_CLOCK_OBJECT_ID\" --gas-budget 100000000 --json"; then
...
echo "$TX_JSON" | jq '{
  ...
  events: [.events[] | select(.type | contains("DepositCancelled"))]
}'
