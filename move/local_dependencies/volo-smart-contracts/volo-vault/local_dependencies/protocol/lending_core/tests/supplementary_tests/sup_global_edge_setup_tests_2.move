#[test_only]
module lending_core::sup_edge_global_2 {
    use sui::clock;
    ...
    const TEST_COIN_ORACLE_ID: u8 = 0;
    const TEST_COIN_INITIAL_PRICE: u256 = 1_0000;

    const NAVX_DECIMALS: u8 = 9;
    const NAVX_ORACLE_ID: u8 = 5;
    const NAVX_INITIAL_PRICE: u256 = 200000000;
    ...
    public fun init_protocol(scenario_mut: &mut Scenario) {
        ...
        test_scenario::next_tx(scenario_mut, owner);
        {
            ...
            eth2_test::init_for_testing(test_scenario::ctx(scenario_mut));  // Initialization of ETH2 coin
            ...
            navx_test::init_for_testing(test_scenario::ctx(scenario_mut)); // Initialization of navx coin
        ...
        test_scenario::next_tx(scenario_mut, owner);
        {
            ...
            let pool_admin_cap = test_scenario::take_from_sender<PoolAdminCap>(scenario_mut);
            let storage_admin_cap = test_scenario::take_from_sender<StorageAdminCap>(scenario_mut);
            let navx_metadata = test_scenario::take_immutable<CoinMetadata<NAVX_TEST>>(scenario_mut);

            storage::init_reserve<NAVX_TEST>(
                &storage_admin_cap,
                &pool_admin_cap,
                &clock,
                &mut storage,
                NAVX_ORACLE_ID,                                   // oracle id
                false,                                           // is_isolated
                5000000_000000000_000000000000000000000000000,  // supply_cap_ceiling: 5000000
                0,                     // borrow_cap_ceiling: 0%
                0,                                               // base_rate: 0%
                500000000000000000000000000,                     // optimal_utilization: 50%
                400000000000000000000000000,                      // multiplier: 40%
                3200000000000000000000000000,                    // jump_rate_multiplier: 320%
                200000000000000000000000000,                     // reserve_factor: 20%
                200000000000000000000000000,                     // ltv: 20%
                100000000000000000000000000,                     // treasury_factor: 10%
                350000000000000000000000000,                     // liquidation_ratio: 35%
                50000000000000000000000000,                      // liquidation_bonus: 5%
                250000000000000000000000000,                     // liquidation_threshold: 25%
                &navx_metadata,                                   // metadata
                test_scenario::ctx(scenario_mut)
            );

            clock::destroy_for_testing(clock);
            test_scenario::return_shared(storage);
            test_scenario::return_immutable(navx_metadata);
            test_scenario::return_to_sender(scenario_mut, pool_admin_cap);
            test_scenario::return_to_sender(scenario_mut, storage_admin_cap);
        };

        // Protocol: Adding ETH2 pools
        test_scenario::next_tx(scenario_mut, owner);
        {
            let storage = test_scenario::take_shared<Storage>(scenario_mut);
            let clock = clock::create_for_testing(test_scenario::ctx(scenario_mut));
            let pool_admin_cap = test_scenario::take_from_sender<PoolAdminCap>(scenario_mut);
            let storage_admin_cap = test_scenario::take_from_sender<StorageAdminCap>(scenario_mut);
            let eth_metadata = test_scenario::take_immutable<CoinMetadata<ETH2_TEST>>(scenario_mut);
