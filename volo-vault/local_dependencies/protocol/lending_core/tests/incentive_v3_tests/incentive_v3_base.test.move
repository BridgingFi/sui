...
module lending_core::incentive_v3_test {
    ...
    #[test_only]
    public fun prepare_user_assets(c: &Clock, user_a: &mut Scenario, user_b: &mut Scenario) {
        next_tx(user_a);
        {
            deposit<SUI_TEST>(user_a, c, 10000)
        };

        next_tx(user_b);
        {
            deposit<SUI_TEST>(user_b, c, 990000)
        };
    }
    ...
    public fun test_create_incentive_v3() {
        ...
        let scenario_c = test_scenario::begin(USERC);
        ...
        next_tx(&mut scenario);
        {
            ...
            prepare_user_assets(&clock, &mut scenario_a, &mut scenario_b);
        };
        ...
        {
            withdraw<SUI_TEST>(&mut scenario_a, &clock, 10000);
            ...
            assert_user_reward<SUI_TEST, SUI_TEST>(&mut scenario_a, 1, 25000000000000000000000, 25000000000000000000000, 250000000, 0);
        };

        // UserC Deposit 10000 After 10s -> 20s
        clock::increment_for_testing(&mut clock, 10 * 1000);
        {
            deposit<SUI_TEST>(&mut scenario_c, &clock, 1000);
            ...
            assert_user_reward<SUI_TEST, SUI_TEST>(&mut scenario_c, 1, 50252525252525252525252, 50252525252525252525252, 0, 0);
        };

        // UserB Withdraw All After 10s -> 30s
        clock::increment_for_testing(&mut clock, 10 * 1000);
        {
            withdraw<SUI_TEST>(&mut scenario_b, &clock, 990000);
            ...
            assert_user_reward<SUI_TEST, SUI_TEST>(&mut scenario_b, 1, 75479568643039884210418, 75479568643039884210418, 74724772957, 0);
        };

        // UserB Deposit 1000 After 10s -> 40s
        clock::increment_for_testing(&mut clock, 10 * 1000);
        {
            deposit<SUI_TEST>(&mut scenario_b, &clock, 1000);
            ...
            assert_user_reward<SUI_TEST, SUI_TEST>(&mut scenario_b, 1, 25075479568643039884210418, 25075479568643039884210418, 74724772957, 0);
        };

        // UserC Withdraw 1000 After 10s -> 50s
        clock::increment_for_testing(&mut clock, 10 * 1000);
        {
            withdraw<SUI_TEST>(&mut scenario_c, &clock, 1000);
            ...
            assert_user_reward<SUI_TEST, SUI_TEST>(&mut scenario_c, 1, 37575479568643039884210418, 37575479568643039884210418, 37525227043, 0);
        };

        // UserB Claim Rewards After 10s -> 60s
        clock::increment_for_testing(&mut clock, 10 * 1000);
        {
            claim_reward<SUI_TEST, SUI_TEST>(&mut scenario_b, &clock, 1);
            ...
            assert_user_reward<SUI_TEST, SUI_TEST>(&mut scenario_b, 1, 62575479568643039884210418, 62575479568643039884210418, 112224772957, 112224772957);
        };

        next_tx(&mut scenario_b);
        {
            let reward_fund = test_scenario::take_shared<RewardFund<SUI_TEST>>(&scenario_b);
            let balance_value = incentive_v3::get_balance_value_by_reward_fund(&reward_fund);
