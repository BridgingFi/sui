...
module lending_core::incentive_tests {
    ...
    public fun test_add_pool_by_other() {
        ...
        let other_scenario = test_scenario::begin(other);
        {
            global::init_protocol(&mut scenario);
        };

        test_scenario::next_tx(&mut other_scenario, other);
        {
            let i = test_scenario::take_shared<Incentive>(&scenario);

            let ctx = test_scenario::ctx(&mut other_scenario);
        ...
        test_scenario::end(other_scenario);
    }
