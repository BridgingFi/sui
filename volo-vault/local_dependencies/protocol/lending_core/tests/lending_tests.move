...
module lending_core::lending_tests {
    ...
    public fun test_add_ltv_to_borrow_health_factor() {
        ...
        test_scenario::next_tx(&mut scenario, OWNER);
        {
            ...
            let clock = clock::create_for_testing(test_scenario::ctx(&mut scenario));
            let eth_coin = coin::mint_for_testing<ETH_TEST>(1_000000000, test_scenario::ctx(&mut scenarioB));
            base_lending_tests::base_deposit_for_testing(&mut scenarioB, &clock, &mut eth_pool, eth_coin, 3, 1_000000000);
    ...
    public fun test_add_ltv_to_borrow_health_factor_multiple_assets() {
        ...
        // userA deposit 1000k sui
        test_scenario::next_tx(&mut scenarioA, userA);
        {
            let usdt_pool = test_scenario::take_shared<Pool<USDT_TEST>>(&scenario);
            let clock = clock::create_for_testing(test_scenario::ctx(&mut scenario));

            let usdt_coin = coin::mint_for_testing<USDT_TEST>(1000000_000000, test_scenario::ctx(&mut scenarioA));
            base_lending_tests::base_deposit_for_testing(&mut scenarioA, &clock, &mut usdt_pool, usdt_coin, 2, 1000000_000000);
        ...
        test_scenario::next_tx(&mut scenarioA, userA);
        {
            ...
            let test_coin = coin::mint_for_testing<TEST_COIN>(1000000_0000, test_scenario::ctx(&mut scenarioA));
            base_lending_tests::base_deposit_for_testing(
                &mut scenarioA,
                &clock,
                &mut test_coin_pool,
        ...
        test_scenario::next_tx(&mut scenario, OWNER);
        {
            ...
            let usdc_coin = coin::mint_for_testing<USDC_TEST>(1000_000000, test_scenario::ctx(&mut scenarioB));
            base_lending_tests::base_deposit_for_testing(
                &mut scenarioB,
                &clock,
                &mut usdc_pool,
        ...
        test_scenario::next_tx(&mut scenario, OWNER);
        {
            let sui_pool = test_scenario::take_shared<Pool<SUI_TEST>>(&scenario);
            let clock = clock::create_for_testing(test_scenario::ctx(&mut scenarioB));
            let sui_coin = coin::mint_for_testing<SUI_TEST>(10000_000000000, test_scenario::ctx(&mut scenarioB));
            base_lending_tests::base_deposit_for_testing(
                &mut scenarioB,
                &clock,
                &mut sui_pool,
    ...
    public fun test_repay_on_behalf_of_should_success() {
        ...
        { // UserA and UserB Perform Supply And Borrow Operations
            ...
            test_scenario::next_tx(&mut user_b_scenario, UserB);
            {
                ...
                base_lending_tests::base_borrow_for_testing(&mut user_b_scenario, &test_clock, &mut usdc_pool, 1, 1000_000000);
                test_scenario::return_shared(usdc_pool);
            };
        };
    ...
    public fun test_on_behalf_of_10_years() {
        ...
        test_scenario::next_tx(&mut user_b_scenario, UserB);
        {
            ...
            base_lending_tests::base_borrow_for_testing(&mut user_b_scenario, &test_clock, &mut sui_pool, 0, 30_000000000);
        ...
        // repay 30
        test_scenario::next_tx(&mut user_b_scenario, UserA);
        {
            let sui_pool = test_scenario::take_shared<Pool<SUI_TEST>>(&scenario);
    ...
    public fun test_normal_10_years() {
        ...
        test_scenario::next_tx(&mut user_a_scenario, UserA);
        {
            ...
            let sui_coin = coin::mint_for_testing<SUI_TEST>(100_000000000, test_scenario::ctx(&mut user_a_scenario));
            let sui_value = coin::value(&sui_coin);
            base_lending_tests::base_deposit_for_testing<SUI_TEST>(&mut user_a_scenario, &test_clock, &mut sui_pool, sui_coin, 0, sui_value);
        ...
        test_scenario::next_tx(&mut user_a_scenario, UserA);
        {
            ...
            base_lending_tests::base_borrow_for_testing(&mut user_a_scenario, &test_clock, &mut sui_pool, 0, 30_000000000);
        ...
        test_scenario::next_tx(&mut user_a_scenario, UserA);
        {
            ...
            let sui_coin = coin::mint_for_testing<SUI_TEST>(30_000000000, test_scenario::ctx(&mut user_a_scenario));
            let sui_value = coin::value(&sui_coin);
            base_lending_tests::base_repay_for_testing<SUI_TEST>(&mut user_a_scenario, &test_clock, &mut sui_pool, sui_coin, 0, sui_value);
