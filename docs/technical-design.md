...
- All functions accept generic `CoinType` to avoid hard-coding the USDC type while still letting the frontend target USDC.

## External Dependencies
- Git dependency on `https://github.com/Sui-Volo/volo-smart-contracts.git` (`volo-vault` subdir) declared in `Move.toml`.
- No custom copy of Volo bytecode; the package is imported during build to preserve audit guarantees.
